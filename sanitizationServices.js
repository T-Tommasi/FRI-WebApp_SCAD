...
class SanitizationServices {
  ...
  static sanitizeMoney(amount, uuid) {
    if (typeof amount === "number" && isFinite(amount)) {
      Logger.log("----");
      Logger.log(
        `import ${amount} from invoice ${uuid} already a number, returning`
      );
      Logger.log("----");
      return amount;
    }
    ...
    if (amount === null || amount == "") {
      Logger.log(
        `SanitizeMoney (for UUID: ${uuid}): Received empty/null value. Returning 0.`
      );
      return 0;
    }
  ...
  static sanitizeClientVendorId(uuid) {
    ...
    for (let trim of trimmedID) {
      if (
        ...
        Logger.log(`----`);
        Logger.log(`Skipped trim ${trim} as it's a identifier`);
